    ...
    if (type === 'load') {
        ...
        vadModel = await AutoModel.from_pretrained('onnx-community/silero-vad', { 
            ...
            progress_callback: (prog) => {
                ...
                if (!fileName) fileName = 'vad.onnx';
                
                const now = Date.now();
                if (prog.status === 'progress' && (now - lastProgressTime < 50)) return;
        ...
        port.onmessage = async (we) => {
            if (we.data.type === 'vad' && vadModel) {
                vadQueue.push(we.data);
                if (!isProcessingVAD) {
                    isProcessingVAD = true;
                    while (vadQueue.length > 0) {
                        const currentData = vadQueue.shift();
                        try {
                            const chunk = new Float32Array(currentData.data);
                            const rawRms = currentData.rawRms || 0;
                            if (isSpeaking) { 
